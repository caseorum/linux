...
int bch2_write_index_default_trans(struct btree_trans *trans, struct bch_write_op *op,
				   struct bkey_buf *sk)
{
	...
	do {
		k = bch2_keylist_front(&op->insert_keys);

		k->k.p.snapshot = snapshot;

		bch2_bkey_buf_copy(sk, trans->c, k);
	...
	} while (!ret && !bch2_keylist_empty(&op->insert_keys));
...
int bch2_write_index_default(struct bch_write_op *op)
{
	...
	do {
		BUG_ON(bch2_keylist_empty(&op->insert_keys));

		bch2_trans_begin(&trans);
		ret = bch2_write_index_default_trans(&trans, op, &sk);
	} while (ret == -EINTR);
